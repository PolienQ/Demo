 ...
 * @author pans
 * @date 2016/8/7
 * @describ 地址: http://blog.csdn.net/vipzjyno1/article/details/25248021
 */
...
//		notification.flags = Notification.FLAG_AUTO_CANCEL; //在通知栏上点击此通知后自动清除此通知
        ...
        //注意：这里的这个APK是放在assets文件夹下，获取路径不能直接读取的，要通过COYP出去在读或者
        // 直接读取自己本地的PATH，这边只是做一个跳转APK，实际打不开的
...
//		Uri uri = Uri.parse(apk_path);
    ...
    void onClickBigStyleShow(View view) {
        showBigStyleNotify();
    }

    @OnClick(R.id.btn_show_progress)
    void onClickShowProgress(View view) {
        presentController(ProgressActivity.class);
    }

    @OnClick(R.id.btn_show_resident)
    void onClickShowResident(View view) {
        showCzNotify();
    }
